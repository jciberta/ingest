...
class BorsaTreball extends Objecte
{
   ...
   */
  function __construct($conn = null, $usuari = null, $sistema = null, $professorGestor = null)
  {
    parent::__construct($conn, $usuari, $sistema);
    $this->professorGestor = $professorGestor;
  }
    ...
    while ($row = $resultSet->fetch_assoc()) {
      if ($row["publicat"] === 1) {
        ...
        <tr>
          ...
          ";
        if ($this->professorGestor !== null && $this->professorGestor->esGestorBorsa()) {
          $output .= "
            <button type='button' class='btn btn-primary' data-bs-toggle='modal' data-bs-target='#modalEditaOferta' onclick='editaOferta($row[borsa_treball_id])'>
